...
```
implementations/<slug>/CLAUDE.md
```
...
1. Read `implementations/<slug>/prd.json` to find the current state of all slices
2. Read `implementations/<slug>/progress.txt` for learnings from previous iterations
...
5. Commit, update `implementations/<slug>/prd.json`, append to `implementations/<slug>/progress.txt`
...
Output exactly `<promise>COMPLETE</promise>` (on its own line) when all slices in `implementations/<slug>/prd.json` are either `passes: true` or `"aborted"`. This signals Ralph to stop looping.
